...
子类继承父类的非私有属性和方法，体现 "is-a" 关系（如狗是动物）。
...
子类继承父类后，根据自身需求重写父类方法（方法名、参数列表、返回值完全一致）。
...
public class MethodOverrideDemo {
    public static void main(String[] args) {
        ...
        circle.radius = 2.0;
        circle.color = "红色";
        System.out.println("红色圆形面积：" + circle.calculateArea()); // 输出：12.566...

        Rectangle rect = new Rectangle();
        rect.width = 3.0;
        rect.height = 4.0;
        rect.color = "蓝色";
        System.out.println("蓝色矩形面积：" + rect.calculateArea()); // 输出：12.0
    }
}
```
...
子类构造方法必须先调用父类构造方法（默认调用无参构造，可通过super()显式调用带参构造）。
...
class Person {
    ...
    // 父类无参构造
    public Person() {
        System.out.println("Person无参构造被调用");
    }
    ...
    public Person(String name, int age) {
        this.name = name;
        this.age = age;
        System.out.println("Person带参构造被调用：" + name + "," + age);
    }
}
...
class Student extends Person {
    ...
    public Student(String name, int age, String studentId) {
        ...
        System.out.println("Student带参构造被调用：" + studentId);
    }

    // 子类方法
    public void study() {
        System.out.println(studentId + "号学生" + name + "正在学习");
    }
}
...
public class ConstructorInheritanceDemo {
    public static void main(String[] args) {
        // 调用子类无参构造
        System.out.println("=== 创建无参学生对象 ===");
        Student s1 = new Student(); // 先执行父类无参构造，再执行子类无参构造

        // 调用子类带参构造
        System.out.println("\n=== 创建带参学生对象 ===");
        Student s2 = new Student("张三", 18, "2025001");
        s2.study(); // 输出：2025001号学生张三正在学习
    }
}
```
...
基于继承实现多态，父类引用可指向任意子类对象，调用方法时自动执行子类重写的实现。

```java
// 父类：交通工具类
abstract class Vehicle { // 抽象类（也可使用普通类，抽象类更贴合多态场景）
    // 抽象方法（子类必须重写）
...
class Car extends Vehicle {
    @Override
    public void run() {
        System.out.println("汽车：四个轮子在路上行驶");
    }
}
...
class Plane extends Vehicle {
    @Override
    public void run() {
        System.out.println("飞机：机翼滑翔在天空飞行");
    }
}
...
class Bicycle extends Vehicle {
    @Override
    public void run() {
        System.out.println("自行车：两个轮子靠人力骑行");
    }
}
...
public class PolymorphismDemo {
    public static void main(String[] args) {
        ...
        // 调用方法时，自动执行子类重写的实现（多态特性）
        v1.run(); // 输出：汽车：四个轮子在路上行驶
        v2.run(); // 输出：飞机：机翼滑翔在天空飞行
        v3.run(); // 输出：自行车：两个轮子靠人力骑行
    }
}
```
...
抽象类不能实例化，子类继承后必须实现所有抽象方法，强制子类遵循统一规范。
...
abstract class Employee {
    ...
    // 抽象方法（子类必须实现）
    public abstract double calculateSalary(); // 计算薪资（不同员工计算规则不同）
}
...
// 测试类
public class AbstractClassInheritanceDemo {
    public static void main(String[] args) {
        RegularEmployee re = new RegularEmployee();
        re.empId = "E001";
        re.empName = "李四";
        re.baseSalary = 8000.0;
        re.showInfo();
        System.out.println("普通员工月薪：" + re.calculateSalary()); // 输出：8000.0

        Manager m = new Manager();
        m.empId = "M001";
        m.empName = "王五";
        m.baseSalary = 15000.0;
        m.bonus = 5000.0;
        m.showInfo();
        System.out.println("经理月薪：" + m.calculateSalary()); // 输出：20000.0
    }
}
```
